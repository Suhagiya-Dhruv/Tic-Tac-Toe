const player1name = sessionStorage.getItem('p1');
const player2name = sessionStorage.getItem('p2');
        ...
        if (this.getAttribute("checked") != "true") {
            ...
            if (count % 2 == 0) {
                ...
                if (check(arrplayer2) && player2 >= 3) {
                    winningname.innerText = player2name;
...
function click(name) {
    title.innerText = "Congratulations " + name + " You have won the Game";
    for (i = 0; i < aer.length; i++) {
        aer[i].onclick = function() {
            var b = Number(this.id);
            this.setAttribute("checked", "true")
